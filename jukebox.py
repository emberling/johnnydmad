# JUKEBOX - python insertion wrapper for the FF6 Music Player hack by madsiur
# building on work by Lenophis and tsushiy
# https://www.ff6hacking.com/wiki/doku.php?id=ff3:ff3us:patches:madsiur:music_player

# *** READ THIS BEFORE EDITING THIS FILE ***

# This file is part of the johnnydmad project.
# ( https://github.com/emberling/johnnydmad )
# johnnydmad is designed to be used inside larger projects, e.g.
# Beyond Chaos, Beyond Chaos Gaiden, or potentially others in the future.
# If you are editing this file as part of "Beyond Chaos" or any other
# container project, please respect the independence of these projects:
# - Keep johnnydmad project files in a subdirectory, and do not modify
#   the directory structure or mix in arbitrary code files specific to
#   your project.
# - Keep changes to johnnydmad files in this repository to a minimum.
#   Don't make style changes to code based on the standards of your
#   containing project. Don't remove functionality that you feel your
#   containing project won't need. Keep it simple so that code and
#   changes can be easily shared across projects.
# - Major changes and improvements should be handled through, or at
#   minimum shared with, the johnnydmad project, whether through
#   submitting changes or through creating a fork that other johnnydmad
#   maintainers can easily see and pull from.

import os

# This construction is required for devtool functionality. Do not remove.
try:
    from .musicrandomizer import open_resource
    from .mfvitools.insertmfvi import byte_insert, int_insert
except ImportError:
    from musicrandomizer import open_resource
    from mfvitools.insertmfvi import byte_insert, int_insert

JUKEBOX_DEFAULT_OFFSET = 0x306000
JUKEBOX_TITLE_LENGTH = 0x1A
RANDOMIZED_TRACKID_LENGTH = 8
RANDOMIZED_TITLE_LENGTH = 18

def generate_data_chunks(jukebox_config = None, offset = JUKEBOX_DEFAULT_OFFSET):
    # External jukebox configs should be a list of strings of the form
    # ID: playlist_id, jukebox_label
    # just as seen in jukebox_config.txt. Example:
    # 01: prelude, pre:
    # The order of these strings determines the order displayed in the jukebox.
    
    if not jukebox_config:
        cfgpath = os.path.join('tables', 'jukebox_config.txt')
        try:
            with open_resource(cfgpath, "r") as f:
                jukebox_config = f.readlines()
        except IOError:
            print(f"could not open {cfgpath}, jukebox generation failed")
            jukebox_config = [""]
    
    data_titles = bytearray()
    data_pointers = bytearray()
    data_order = bytearray()
    
    titles = {}
    
    for line in jukebox_config:
        # trim comments
        line = line.split('#')[0]
        # reject blanks and other lines without a xx: ~~ structure
        if ':' not in line: continue
        # finish splitting
        id, line = [s.strip() for s in line.split(':', 1)]
        if ',' in line:
            name, entry = [s.strip() for s in line.split(',', 1)]
        else:
            name = line.strip()
            entry = ""
        # reject bad ids
        if not set(id) <= set("0123456789abcdefABCDEF"):
            print(f"warning: jukebox config ({i}): id {id} contains invalid characters")
            continue
        id = int(id, 16)
        if id == 0:
            continue
            
        titles[id] = entry
        data_order += int.to_bytes(id, 1, "little")
    data_order += b"\x00"
    
    # Putting a lower bound on jukebox data size (not displayed lines),
    # mainly because I don't want to deal with figuring out what happens if it's zero
    max_id = max(max(titles.keys()), 0x10)
    this_offset = offset % 0x10000
    
    for id in range(1, max_id + 1):
        this_title = titles[id] if id in titles else ""
        data_titles += convert_text(this_title, JUKEBOX_TITLE_LENGTH) + b"\x00"
        
        data_pointers += int.to_bytes(this_offset, 2, "little")
        this_offset = (offset % 0x10000) + len(data_titles)
        
    
    return data_titles, data_pointers, data_order
    
def add_music_player(rom, song_titles, jukebox_title = "JOHNNY D.MAD JUKEBOX",
        jukebox_config = None, offset = JUKEBOX_DEFAULT_OFFSET):
            
    mp_offset = offset
    # mp_data_chunk = b""
    mp_program_chunk = b""
    
    mp_program_chunk = byte_insert(mp_program_chunk, 0x127, convert_text(jukebox_title, size=20))
    
    title_chunk, pointers_chunk, order_chunk = generate_data_chunks(jukebox_config, offset)
    mp_data_chunk = title_chunk + pointers_chunk + order_chunk
    
    pointers_offset = len(title_chunk) + offset
    order_offset = len(pointers_chunk) + pointers_offset
    data_count = len(pointers_chunk) // 2
    
    rom = byte_insert(rom, 0x3011A, b"\x20\x74\xFA\x4C\xBA\x01")
    rom = byte_insert(rom, 0x302D1, b"\x9B\xFA\xD2\xFA")
    # Non-essential code removed due to conflict with another Beyond Chaos loaded hack
    #rom = byte_insert(rom, 0x31DCB, b"\x20\x83\xFA\x64\x26\x60")
    rom = byte_insert(rom, 0x32E6A, b"\x10\xFA")
    rom = byte_insert(rom, 0x32F66, b"\x00\xFA")
    rom = byte_insert(rom, 0x32F77, b"\x00\xFA")
    rom = byte_insert(rom, 0x32F89, b"\x08")
    rom = byte_insert(rom, 0x33175, b"\x80\x04")
    rom = byte_insert(rom, 0x331BE, b"\x0F\x00\x00\x00\x00\x35\x5D\x07\x06")
    rom = byte_insert(rom, 0x3321A, b"\x20\x52\xFA")
    rom = byte_insert(rom, 0x33253, b"\x37\x7E")
    rom = byte_insert(rom, 0x332A0, b"\xBB\x7D")
    rom = byte_insert(rom, 0x332AC, b"\xC1")
    rom = byte_insert(rom, 0x336AE, b"\x20\xFA")
    rom = byte_insert(rom, 0x33741, b"\x45\xFA")
    rom = byte_insert(rom, 0x3376F, b"\x4A\xFA")
    rom = byte_insert(rom, 0x337FD, b"\xB9")
    rom = byte_insert(rom, 0x33804, b"\x77\x7D")
    rom = byte_insert(rom, 0x3380B, b"\xBF\x7D\xC1\x00\xF7")
    rom = byte_insert(rom, 0x3FA00, mp_program_chunk)
    rom = byte_insert(rom, 0x50627, b"\x12\xA5\x05\xF0\x12\xA2\xFF\xFF\xE8\xBF\xF9\x06\xC5\x30\x08\xC5\x01\xD0\xF5\xA9\x04\x04")
    
    rom = byte_insert(rom, mp_offset, mp_data_chunk)
    rom = int_insert(rom, 0x3FBE4, code_offset(order_offset), 3)
    rom = int_insert(rom, 0x3FD44, code_offset(pointers_offset), 3)
    
    for id, title in song_titles.items():
        b = convert_text(title, size = RANDOMIZED_TITLE_LENGTH)
        loc = mp_offset + (id - 1) * (JUKEBOX_TITLE_LENGTH + 1) + RANDOMIZED_TRACKID_LENGTH
        if loc + len(b) >= pointers_offset:
            print(f"WARNING: Attempted out of range jukebox title write")
            print(f"  ID: {id:02X} Title: {title}")
        else:
            rom = byte_insert(rom, loc, b)
        
    return rom

def convert_text(text, size = None):
    if size and len(text) < size:
        text += " " * (size - len(text))
    bin = bytearray()
    for c in text:
        if c in menu_text_table:
            bin += menu_text_table[c]
        else:
            bin += b"\xCF"
    if size:
        bin = bin[:size]
    return bin
    
def code_offset(offset):
    if offset < 0x400000:
        return offset + 0xC00000
    else:
        return offset
        
menu_text_table = {
    'A': b'\x80',
    'B': b'\x81',
    'C': b'\x82',
    'D': b'\x83',
    'E': b'\x84',
    'F': b'\x85',
    'G': b'\x86',
    'H': b'\x87',
    'I': b'\x88',
    'J': b'\x89',
    'K': b'\x8A',
    'L': b'\x8B',
    'M': b'\x8C',
    'N': b'\x8D',
    'O': b'\x8E',
    'P': b'\x8F',
    'Q': b'\x90',
    'R': b'\x91',
    'S': b'\x92',
    'T': b'\x93',
    'U': b'\x94',
    'V': b'\x95',
    'W': b'\x96',
    'X': b'\x97',
    'Y': b'\x98',
    'Z': b'\x99',
    'a': b'\x9A',
    'b': b'\x9B',
    'c': b'\x9C',
    'd': b'\x9D',
    'e': b'\x9E',
    'f': b'\x9F',
    'g': b'\xA0',
    'h': b'\xA1',
    'i': b'\xA2',
    'j': b'\xA3',
    'k': b'\xA4',
    'l': b'\xA5',
    'm': b'\xA6',
    'n': b'\xA7',
    'o': b'\xA8',
    'p': b'\xA9',
    'q': b'\xAA',
    'r': b'\xAB',
    's': b'\xAC',
    't': b'\xAD',
    'u': b'\xAE',
    'v': b'\xAF',
    'w': b'\xB0',
    'x': b'\xB1',
    'y': b'\xB2',
    'z': b'\xB3',
    '0': b'\xB4',
    '1': b'\xB5',
    '2': b'\xB6',
    '3': b'\xB7',
    '4': b'\xB8',
    '5': b'\xB9',
    '6': b'\xBA',
    '7': b'\xBB',
    '8': b'\xBC',
    '9': b'\xBD',
    '!': b'\xBE',
    '?': b'\xBF',
    '/': b'\xC0',
    ':': b'\xC1',
    '"': b'\xC2',
    "'": b'\xC3',
    '-': b'\xC4',
    '.': b'\xC5',
    ',': b'\xC6',
    '_': b'\xC7', # ellipsis
    ';': b'\xC8',
    '#': b'\xC9',
    '+': b'\xCA',
    '(': b'\xCB',
    ')': b'\xCC',
    '%': b'\xCD',
    '~': b'\xCE', # tilde
    '*': b'\xCF', # asterisk
    '=': b'\xD2',
#   '„': b'\xD3', # (0132) two dot ellipsis
    '^': b'\xD4', # up arrow
    '>': b'\xD5', # right arrow
    '<': b'\xD6', # down left arrow                           
    '&': b'\xD7', # weird x
    '`': b'\xD8', # dagger
#   'Š': b'\xD9', # (0138) sword
    '|': b'\xDA', # spear
    '\\': b'\xDB', # katana
#   'ƒ': b'\xDC', # (0131) staff
#   '‹': b'\xDD', # (0139) brush
    '}': b'\xDE', # shuriken
    '@': b'\xDF', # flail
    '$': b'\xE0', # gambler
#   '€': b'\xE1', # (0128) claw
#   '†': b'\xE2', # (0134) shield
#   'Œ': b'\xE3', # (0140) helmet
#   '‡': b'\xE4', # (0135) armor 
#   '™': b'\xE5', # (0153) tools
#   '‰': b'\xE6', # (0137) scroll
    '{': b'\xE7', # ring
    '[': b'\xE8', # white magic
    ']': b'\xE9', # black magic
#   '•': b'\xEA', # (0149) gray magic
    ' ': b'\xFE'
    }
